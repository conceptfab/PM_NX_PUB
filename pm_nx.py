...
from cfab_mod_tools import (check_settings_file, check_settings_folder,
                            check_work_folder, project_json_read,
                            projects_list, read_settings, set_settings_file)
from cfab_mod_view import view_long_projects_list
...
def input_choice(settings_list, strings):
    ...
    print(strings[10] + strings[11] + '\n'  # Czy chcesz dodać nowy projekt?  ( T )
          # Aktualizować listę projektów?  ( A )
          + strings[12] + strings[13] + '\n'
          # Wyświetlić rozszerzoną listę projektów?  ( W )
          + strings[26] + strings[27] + '\n'
          + strings[14] + strings[15])  # Czy opuścić program?  ( Q )
...
def select_choice(choice, settings_list, strings):
    ...
    if choice in ('T', 't'):
        ...
        view_long_projects_list(settings_list[0], projects_dict)
        input_choice(settings_list, strings)

    elif choice in ('A', 'a'):
        logging.debug('MENU - input choice - AKTUALIZACJA')
        # update()
        input_choice(settings_list, strings)
...
def run_main():
    ...
    work_folder_status = check_work_folder(
        settings_list[0])  # TODO dodać wyjątki
